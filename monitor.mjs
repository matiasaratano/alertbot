...
export const MONITOR = {evidenceWindow:3,structureBars:3,recoveryBars:2};

// Distintas lecturas del mismo oscilador no se suman como votos separados.
// Nivel 1: debilitamiento. Nivel 2: además cede estructura de precio reciente.
export function classifyWeakness({side,momentum,rsiWarning,divergence,priceBreak}) {
 const evidence=[];
 ...
 const level=priceBreak&&(momentum||rsiWarning||divergence)?2:divergence||(momentum&&rsiWarning)?1:0;
...
export function buildWeakness(candles,p=INDICATOR,options=MONITOR) {
 ...
 const {val}=computeSqueezeMomentum(high,low,close,p.sqz),rsi=rsiWilder(close,p.rsiLen);
 ...
 for(const side of ['long','short']) {
  ...
  for(let i=0;i<n;i++) {
   ...
   results[side].push({...classifyWeakness({side,momentum,rsiWarning,divergence,priceBreak}),idx:i,recovered,price:close[i],rsi:rsi[i],
