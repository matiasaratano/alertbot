...
test('watch commands validate universe, side, arity, are idempotent and remove all watch state',()=>{
 const s={};applyWatchCommand(s,'/seguir BTCUSD 4h long',t);assert.equal(listWatches(s).length,1);
 applyWatchCommand(s,'/seguir BTCUSD 4h long',t+100);assert.equal(s['_watch:BTCUSD:4h:started'],t);
 applyWatchCommand(s,'/seguir BTCUSD 4h short',t+200);assert.equal(listWatches(s)[0].side,'short');
 ...
 applyWatchCommand(s,'/dejar BTCUSD 4h',t);assert.deepEqual(s,{});
});
test('commands accept only configured private owner or explicit group user; offset survives replay',async()=>{
 const state={},replies=[];
 const update=(id,chat=7,from=7,type='private')=>({update_id:id,message:{chat:{id:chat,type},from:{id:from},date:t/1000,text:'/seguir BTCUSD 4h long'}});
...
test('command persistence failure cannot consume offset; reply failure cannot undo applied command',async()=>{
 const state={},updates=[{update_id:1,message:{chat:{id:7,type:'private'},from:{id:7},date:t/1000,text:'/seguir BTCUSD 4h long'}}];
...
test('monitor starts from registration, persists only successful alerts, and escalates without entry cooldown',async()=>{
 const s={};applyWatchCommand(s,'/seguir BTCUSD 4h long',t);const w=listWatches(s)[0],c=candles();let sent=0;
 ...
 assert.equal(s['_watch:BTCUSD:4h:cursor'],t);
...
test('backlog includes transient danger as one historical notice, without pretending it is current',async()=>{
 const s={};applyWatchCommand(s,'/seguir BTCUSD 4h long',t);const w=listWatches(s)[0],c=candles(4);let sent=0;
 const series=[event(0),event(2),event(1),event(0)];
 await processWatch({state:s,watch:w,candles:c,series,now:c.closeTime[3],persist:()=>{},send:async text=>{sent++;assert.match(text,/AVISO RECUPERADO/);assert.match(text,/sin condición de debilidad/);}});assert.equal(sent,1);assert.equal(s['_watch:BTCUSD:4h:level'],2);
});
...
test('scanner consults active 4h watch first and reuses data for opportunity target',async()=>{
 const now=Date.parse('2026-09-15T16:02:00Z'),state={};applyWatchCommand(state,'/seguir ETHUSD 4h short',now-10000000);
 const calls=[],fetch=async(symbol,tf)=>{
  calls.push([symbol,tf]);const duration=14400000,end=Math.floor((now-60000)/duration)*duration;
 ...
 const result=await scanMarkets({state,clock:()=>now,cryptoSymbols:['BTCUSD'],stockSymbols:[],timeframes:['4h'],extraTargets:[{symbol:'ETHUSD',tf:'4h'}],cryptoFetch:fetch,persist:()=>{},send:async()=>{}});
 assert.equal(result.healthy,true);assert.deepEqual(calls,[['ETHUSD','4h'],['BTCUSD','4h']]);
});
...
test('watch persistence encrypts positions and commands separately from public scanner state',async()=>{
 ...
 const priv={'_watch:BTCUSD:4h:side':1,_telegramUpdateOffset:100};
 ...
 const dir=mkdtempSync(tmpdir()+'/alertbot-watch-test-');try{
  ...
  persist({...priv,'v2:BTCUSD:4h:setup_bull':10});assert.deepEqual(pub,{'v2:BTCUSD:4h:setup_bull':10});
...
test('three-hour backlog warns once about recovered weakness and rearms after two recovery closes',async()=>{
 const s={};applyWatchCommand(s,'/seguir BTCUSD 4h long',t);const w=listWatches(s)[0],c=candles(13);
 ...
 assert.equal(s[watchPrefix('BTCUSD','4h')+'level'],0);
...
test('historical monitor send failure preserves cursor and can retry; expired warnings stay silent',async()=>{
 const s={};applyWatchCommand(s,'/seguir BTCUSD 4h long',t);const w=listWatches(s)[0],c=candles(20);
 ...
 const expired={};applyWatchCommand(expired,'/seguir BTCUSD 4h long',t);
...
test('commands wait through three-hour schedule, expire after four hours, and register from processing time',async()=>{
 const s={},replies=[];
 const make=(id,age)=>({update_id:id,message:{chat:{id:7,type:'private'},from:{id:7},date:(t-age)/1000,text:'/seguir BTCUSD 4h long'}});
...
test('15m watches are inactive, cannot be added and can still be removed',()=>{
 ...
 applyWatchCommand(state,'/seguir BTCUSD 4h long',t);assert.equal(listWatches(state)[0].tf,'4h');
});
